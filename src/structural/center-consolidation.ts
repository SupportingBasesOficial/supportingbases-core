...
export function groupByCenter(obligations: Array<{
  center_id: string;
  amount: number;
  layer: 1 | 2 | 3;
  weighted_amount: number;
}>): Record<string, CenterAggregation> {
  const map: Record<string, CenterAggregation> = {};
  ...
  for (const o of obligations) {
    ...
    if (o.layer === 1) map[id].layer1_exposure += o.weighted_amount;
    if (o.layer === 2) map[id].layer2_exposure += o.weighted_amount;
    if (o.layer === 3) map[id].layer3_exposure += o.weighted_amount;
  }
